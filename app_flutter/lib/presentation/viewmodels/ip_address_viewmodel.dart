...
/// Realises: [Feat-020/IpAddressViewModel]
///
/// State holder for [IpAddressTypes] UI interactions.
///
/// Manages loading, persistence, and mutation of IP address values
/// via an injected [IpAddressRepository]. Notifies listeners on
/// every state transition for reactive UI binding.
class IpAddressViewModel extends ChangeNotifier {
  /// Creates an [IpAddressViewModel] backed by [repository].
  IpAddressViewModel(this._repository);

  final IpAddressRepository _repository;
  IpAddressTypes? _model;
  bool _isLoading = false;
  String? _errorMessage;
  String _currentRecordId = 'default';
  bool _disposed = false;

  /// The currently loaded [IpAddressTypes] model, or null.
  IpAddressTypes? get model => _model;
  ...
  /// Loads the IP address record for [recordId] from the repository.
  Future<void> load(String recordId) async {
    _currentRecordId = recordId;
    _setLoading(true);
    final result = await _repository.fetch(id: recordId);
    switch (result) {
      case Success<IpAddressTypes>(:final value):
        _model = value;
        _errorMessage = null;
      case Failure<IpAddressTypes>(:final error):
        _model = null;
  ...
  Future<void> save(IpAddressTypes record,
  ...
  Future<void> update(IpAddressTypes record,
      {String recordId = 'default'}) async {
